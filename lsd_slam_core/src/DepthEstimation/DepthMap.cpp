...
namespace lsd_slam
{
	...
	{
		const float* keyFrameMaxGradBuf = activeKeyFrame->maxGradients(0);//最底层金字塔图像，就是原始图像的每个像素的梯度值
		...
		for(int y=yMin;y<yMax; y++)
			for(int x=3;x<width-3;x++)
			{
				...
				if(hasHypothesis && keyFrameMaxGradBuf[idx] < MIN_ABS_GRAD_DECREASE)//如果这个点有效，并且梯度值小于MIN_ABS_GRAD_DECREASE（5）
				{
					target->isValid = false;//就跳过
					continue;
				}
	...
	void DepthMap::observeDepth()
	{
		...
		//通过多线程方式调用DepthMap::observeDepthRow进行并行计算各个点的梯度的估计值，等等
...
//更新关键帧
	void DepthMap::updateKeyframe(std::deque< std::shared_ptr<Frame> > referenceFrames)
	{
		...
		oldest_referenceFrame = referenceFrames.front().get();//最老的参考真
		...
		for(std::shared_ptr<Frame> frame : referenceFrames)
		{
			...
			if(frame->pose->trackingParent->frameID == activeKeyFrame->id())
				refToKf = frame->pose->thisToParent_raw;//得到薇姿
			else
				refToKf = activeKeyFrame->getScaledCamToWorld().inverse() *  frame->getScaledCamToWorld();

			frame->prepareForStereoWith(activeKeyFrame, refToKf, K, 0);//为立体匹配做准备
		...
		observeDepth();//通过多线程方式调用DepthMap::observeDepthRow进行并行计算
